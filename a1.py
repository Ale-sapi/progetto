...
def send_to_gemini(text, concise=True):
    # Prompt standard, senza ottimizzazione per JSON/schema
    if concise:
        text = f"Please answer in Italian and answer briefly and concisely, when code is requested answer only and exclusively with the complete code, without explanation, unless otherwise specified, if instead you are asked to complete or correct code answer first with the missing part to be added and then with the complete code of the correction.: {text}"
    else:
